...
/**
 * 컴포넌트 페이지 통합. Designer 영역을 실제 app 과 history 위에서 확인한다.
 */
const at = (url: string) => {
...
describe('컴포넌트 페이지', () => {
  ...
  it('h1 아래에 Designer 영역만 그린다 — docs section 은 없다', () => {
    at('/components/button');
  ...
  it('Button 의 shared scenario 를 Canvas 에 렌더한다', async () => {
    at('/components/button');
  ...
  it('컴포넌트가 아닌 화면에는 Designer 영역이 없다', () => {
    at('/tokens');
...
describe('shell 유지', () => {
  ...
  it('mobile drawer 가 경로 이동 후 닫힌다', async () => {
    at('/components/button');
...
/**
 * 사이드바가 이 앱의 유일한 내비게이션이다. Designer 영역에 같은 목록을 한 번 더 두지 않고,
 * 컴포넌트 검색은 그 목록 위에 있다.
 */
describe('사이드바 컴포넌트 검색', () => {
  ...
  it('검색 후 이동한다', async () => {
    ...
    expect(url()).toBe('/components/popover');
...
describe('Designer Canvas', () => {
  ...
  /** 모든 definition 이 Designer 영역에서 실제로 그려지는지 — 하나라도 비면 여기서 걸린다. */
...
describe('State Matrix', () => {
  ...
  /** pseudo-state 가 cell 로 위조되지 않는다. */
...
describe('Designer 좁은 화면', () => {
  ...
  it('데스크톱 두 열을 줄여 넣지 않고 compact 배치를 쓴다', () => {
    at('/components/button');
  ...
  it('전역 드로어에서 검색해 컴포넌트를 고른다', async () => {
    at('/components/button');
    await userEvent.click(screen.getByTestId('open-menu'));
    const drawer = within(screen.getByTestId('menu-drawer'));
    ...
    expect(url()).toBe('/components/divider');
  ...
  /**
   * 전역 메뉴와 Inspector sheet 는 다른 컨트롤이다 — 이름도 대상도 겹치지 않고, Inspector 트리거를
   * 전역 햄버거로 위장하지도 않는다.
   *
  ...
  it('AppShell 전역 메뉴와 이름·대상이 겹치지 않는다', async () => {
    ...
    // 전역 메뉴는 아이콘 버튼, Inspector 트리거는 글자가 있는 버튼이다.
