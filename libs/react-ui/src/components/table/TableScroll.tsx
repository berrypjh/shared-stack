...
/**
 * 가로로 넘치는 table을 담는 스크롤 viewport.
 * table 시맨틱과 섞지 않는다 — 이것은 뷰포트이고 `Table`은 데이터다.
 * 그래서 별도 컴포넌트이고, 감싸도 table의 역할·이름·헤더 관계가 그대로 남는다.
 * 세 속성이 함께 가야 의미가 있다.
 * - `overflow-x: auto` (CSS): 넘치면 스크롤한다
 * - `tabIndex={0}`: 키보드로도 스크롤할 수 있어야 한다 (WCAG 2.1.1), 휠·드래그만 되는 영역은 키보드 사용자에게 잘린 열이 도달 불가가 되고 CSS만으로는 해결할 수 없다
 * - `role="region"` + 이름: 포커스를 받는 요소는 이름이 있어야 해서 `label`이 필수다
