...
/**
 * BoxedInput 는 `InputBase` 에 윤곽선 클래스를 더하는 시각 래퍼다.
 *
...
describe('<BoxedInput />', () => {
  ...
  it('루트와 입력에 variant 클래스와 InputBase 클래스를 함께 붙인다', () => {
    ...
    expect(screen.getByTestId('root')).toHaveClass(boxedInputClasses.root, inputBaseClasses.root);
    expect(screen.getByTestId('input')).toHaveClass(
      boxedInputClasses.input,
  ...
  it('className 을 variant 루트 클래스와 병합한다', () => {
    ...
    expect(screen.getByTestId('root')).toHaveClass(boxedInputClasses.root, 'custom-root');
  });
  ...
  it('FormControl 상속과 prop override 를 가로막지 않는다', () => {
    render(
      <FormControl error size="sm">
        <BoxedInput data-testid="inherited" />
        <BoxedInput data-testid="overridden" error={false} />
      </FormControl>,
    );
