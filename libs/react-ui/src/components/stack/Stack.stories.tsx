...
/**
 * `Stack`은 1차원 레이아웃 primitive다 — 자식을 한 축으로 흘리는 것이 전부다.
 * 스타일시트가 없고 색·여백·모서리 prop도 없다.
 * 그래서 테마 갤러리 story가 없다 — 테마마다 달라지는 것이 Stack에는 하나도 없어서 비교할 것이 없다.
 * 아래 story의 색은 전부 자식(데모용 상자)이 가진 것이고, 면이 필요하면 `Box`와 겹쳐 쓴다 (`WithBox`).
 * hover·selected·focus story도 없다 — 그런 API가 없다.
 * Stack은 비상호작용이다.
 */
const meta = {
...
/** 데모용 자식. Stack 자체에는 장식 API가 없으므로 시각은 전부 여기에 있다. */
...
/**
 * 축은 두 가지뿐이다.
 * `column`이 기본값인 것은 계약이 정한 것이다 — CSS 기본값(`row`)이 아니라 RN 기본값과 맞춰 두 렌더러가 같게 동작하게 한다.
 */
 ...
 * 계약 어휘는 정규화된 이름(`start`·`end`·`between`)이고 CSS 어휘(`flex-start`·`flex-end`·`space-between`)로 푸는 것은 렌더러 몫이다.
 */
...
/**
 * `wrap`은 boolean이다.
 * 폭이 묶인 틀 안에서 줄바꿈을 확인한다 — 반응형 prop 대신 제약된 컨테이너로 스트레스를 보여 준다 (브레이크포인트 API는 만들지 않는다).
 * `wrap={false}`는 미지정과 다르다.
 * 명시하면 `nowrap` 선언을 만든다.
 */
...
/** Stack은 중첩해서 축을 바꾼다 — 그것이 1차원 primitive로 2차원 화면을 짜는 방법이다 */
...
/**
 * 긴 내용이 틀을 넘칠 때.
 * Stack은 `min-width`를 손대지 않는다 — flex 항목의 기본 `min-width: auto`가 그대로라 가로 축에서 긴 문자열은 줄지 않고 밀어낸다.
 * 그것을 줄이고 싶으면 자식이 정한다 (`minWidth: 0` + `overflow`).
 * 컨테이너가 자식의 축소 정책을 대신 정하지 않는다.
 */
...
/**
 * 면·여백은 `Box`가 가진다.
 * Stack에 `p`·`bg`·`radius`를 두지 않는 이유다 — 두 primitive를 겹쳐 쓰면 각자의 책임이 그대로 남는다.
 */
