 ...
 * 나머지 축은 주지 않으면 선언을 만들지 않는다 — Box의 "미지정은 미적용"과 같은 규칙이고, `gap={0}`·`wrap={false}`는 미지정이 아니라 명시한 값이다.
 * 소비자 `style`이 계산된 레이아웃보다 뒤에 온다.
 * 그래야 escape hatch가 성립한다.
 * 비상호작용이다.
 * `role`·`tabIndex`·`aria-*`를 지어내지 않고 포커스·hover 시각도 만들지 않는다 — 소비자가 준 DOM prop은 그대로 전달한다.
 * `displayName`을 두지 않는다 — 최상위 속성 할당은 tree-shaking을 막는다 (`.size-limit.cjs` 머리말, Avatar·Badge·Chip·List·Table과 같은 관례).
 */
export const Stack = ({
  direction = 'column',
  gap,
  ...
  // Box와 같은 변환을 쓴다 — 토큰이면 `--ds-spacing-*`, 숫자면 px, `0`은 `0px`.
