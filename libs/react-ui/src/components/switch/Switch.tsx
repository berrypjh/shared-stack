...
/**
 * 켜짐/꺼짐 설정.
 * native `<input type="checkbox" role="switch">`를 그대로 쓴다.
 * checked·defaultChecked·disabled·required·name·value·form·FormData·form reset·Space·포커스·라벨 클릭은 native checkbox가 소유하고, 역할만 switch다.
 * input을 숨기지 않는다 — input 자체가 트랙이고 thumb은 CSS가 그린다.
 * 추가 DOM이 없어 접근 가능한 이름에 섞일 것도 없다.
 * FormControl에서는 `disabled`만 상속한다 (`required`는 Checkbox와 같은 이유로 받지 않는다).
 */
