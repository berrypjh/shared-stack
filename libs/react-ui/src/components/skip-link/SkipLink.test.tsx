...
describe('<SkipLink />', () => {
  ...
  /**
   * `href`는 `targetId`를 그대로 이어 붙인다.
   * 인코딩도 검증도 없다 — 그 사실이 계약이라 못박는다.
   * 03에서 인코딩을 넣는다면 이 검사가 먼저 깨져서 드러난다.
   */
  describe('href 계약', () => {
    ...
    it('컨슈머가 href 를 넣어도 targetId 가 이긴다', () => {
      ...
      // 타입은 `href`를 Omit하지만 런타임 우선순위도 같은 방향이어야 한다.
    ...
    it('네이티브 anchor 라 기본으로 탭 순서에 있다', () => {
      ...
      // tabindex를 달지 않는다 — href 있는 anchor는 이미 tabbable이다.
  ...
  /**
   * SkipLink는 CSS가 곧 동작이다.
   * 포커스 전까지 숨고 포커스에서 드러나는 것이 이 컴포넌트의 전부다.
   * JSX에는 그 계약이 없으므로 컴파일된 CSS를 직접 읽는다.
   * jsdom CSSOM은 논리 속성과 `clip`을 신뢰할 수 없게 다뤄서 계산값 대신 텍스트를 본다 (`forcedColors.test.ts`와 같은 이유).
   */
  describe('시각 계약 (컴파일된 CSS)', () => {
    ...
    it('포커스 상태는 논리 속성으로 드러난다', () => {
      ...
      // RTL에서도 따라가야 해서 left/top이 아니라 논리 속성이다.
     ...
     * forced-colors(Windows 고대비) 모드는 저자 색을 시스템 색으로 갈아끼우고 `box-shadow`를 지운다(CSS Color Adjust 1).
     * 그래서 그림자로만 그린 포커스 링은 그 모드에서 통째로 사라진다 — WCAG 2.4.7 위반이다.
     * 살아남는 것은 `outline`이라 `.ui-button:focus-visible`과 같은 전략을 쓴다 — outline과 그림자를 함께 선언해 그림자가 지워져도 outline이 남게 한다.
     * 그 덕에 이 시트에는 forced-colors 전용 블록이 필요 없다.
     */
     ...
     * `:focus-visible`만 두면 브라우저 휴리스틱이 "보이는 포커스"로 치지 않은 경우(프로그래매틱 `focus()`, 일부 스크린리더/키보드 조합)에 bypass link가 포커스를 가진 채로 계속 숨어 있게 된다.
     * 우회 수단이 조용히 사라지는 쪽이 잉여 노출보다 나쁘다.
     */
