...
describe('<Divider />', () => {
  ...
  describe('root', () => {
     ...
     * 역할을 ARIA로 다시 적지 않는 이유이고, 이 검사가 그 전제를 고정한다.
     */
    it('hr 로 렌더링한다', () => {
   ...
   * ARIA는 필요할 때만 적는다 — separator의 기본 방향이 이미 horizontal이다.
   */
  ...
  /**
   * 시각 구분과 시맨틱 구분은 다른 결정이다.
   * 저장소의 기존 구분선은 전부 시각 장식(카드 테두리·행 경계)이다.
   * 그 용도에는 접근성 트리에 separator를 하나 더 만들 이유가 없다.
   * `role="presentation"`은 native 시맨틱만 끄고 요소는 남긴다 — `aria-hidden`과 달리 소비자가 얹은 내용까지 숨기지 않는다.
   */
  describe('decorative', () => {
    it('기본은 시맨틱이다', () => {
      render(<Divider data-testid="divider" />);
  ...
  /**
   * 시각 계약 — 컴파일된 CSS를 텍스트로 읽는다.
   * jsdom의 CSSOM은 논리 속성과 `@media`를 신뢰할 수 있게 다루지 못해서 계산값 대신 텍스트를 본다 (`skip-link`·`switch`와 같은 이유).
   */
  describe('divider.scss', () => {
    ...
    /** UA 기본 `<hr>`은 margin과 inset border를 갖는다. 그것을 지우고 선을 직접 그린다. */
    ...
    /**
     * 선은 `background-color`가 아니라 border로 그린다.
     * forced-colors는 저자 색을 시스템 색으로 갈아끼운다.
     * 1px 배경으로 그린 선은 그 모드에서 배경색과 같아져 사라질 수 있지만 border는 남는다.
     */
    ...
    /** 물리 방향(top/left)이 아니라 논리 속성이라 RTL에서 저절로 뒤집힌다 */
    it('논리 속성만 쓴다', () => {
    ...
    /**
     * forced-colors 대응.
     * `table.scss`와 같은 전략이다 — 그 모드에서만 `CanvasText`로 선을 되살린다.
     * `forced-color-adjust: none`은 쓰지 않는다 (사용자 팔레트를 끄는 셈).
     */
