 ...
 * 그 모드에서 브라우저는 저자 색을 전부 시스템 색으로 갈아끼우고 `box-shadow`를 지운다 (CSS Color Adjust 1).
 * 그래서 두 가지가 조용히 사라진다.
 * - `box-shadow`로 그린 포커스 링·밑줄: 아예 렌더되지 않는다
 * - `border-color` 변화로만 표현한 상태: 평상시와 포커스가 같은 시스템 색으로 평탄화된다
 * 둘이 겹치면 입력의 포커스 표시가 통째로 없어진다 — WCAG 2.4.7 Focus Visible(AA) 위반이다.
 * 살아남는 것은 `outline`이라, 그 모드에서만 outline을 켜서 복구한다.
 * jsdom의 CSSOM은 `@media (forced-colors: active)`를 신뢰할 수 있게 파싱하지 못하므로 컴파일된 CSS를 텍스트로 읽는다.
 * 셀렉터 목록을 박아 두는 대신 "box-shadow로 포커스를 그리는 규칙에는 forced-colors 대응이 있다"는 규칙 자체를 검사한다 — 새 variant가 생기면 목록을 고치지 않아도 함께 걸린다.
 */
 ...
 * `button-base.scss`는 여기 없다 — `:focus-visible`이 outline과 halo를 함께 선언해 forced-colors에서도 outline이 남는다.
 * 버튼은 아래 전용 describe에서 다른 것을 본다.
 */
const SHEETS = [
...
/**
 * 컴파일된 CSS.
 * 주석은 먼저 걷어낸다 — 아래 파서는 `{` 앞을 셀렉터로 보므로, 규칙 위에 달린 주석이 남아 있으면 셀렉터에 딸려 들어간다.
 */
...
/**
 * `selector { body }`를 평평하게 훑는다.
 * 중첩 at-rule은 이 파일 범위에 없다.
 * 그룹 셀렉터(`A, B { … }`)는 하나씩 쪼갠다 — CSS 쪽은 묶어 쓰는 것이 자연스럽고, 검사는 셀렉터 단위여야 어느 쪽이 빠졌는지 이름으로 드러난다.
 */
...
describe.each(SHEETS)('%s', (sheet) => {
  ...
  /**
   * 포커스는 forced-colors에서도 보여야 한다 (WCAG 2.4.7).
   * 검사 대상은 `box-shadow`에만 기대는 규칙이다.
   * 같은 규칙이 이미 `outline`을 함께 선언했다면 outline이 살아남으므로 대응이 필요 없다 — `.ui-button:focus-visible`이 그 경우다.
   * 불필요한 CSS를 요구하지 않도록 그쪽은 제외한다.
   */
 ...
 * `outline:\s*(?!none)` 같은 lookahead는 쓰지 않는다 — `\s*`가 0글자로 되돌아가면 공백 위치에서 lookahead가 통과해 `outline: none`도 "outline이 있다"로 읽힌다.
...
/**
 * 채워진 버튼은 배경색만으로 자기를 식별시킨다.
 * forced-colors에서 배경은 `ButtonFace`, 페이지는 `Canvas`가 되는데 두 값이 같은 고대비 테마가 있어서 버튼이 사라질 수 있다.
...
/**
 * Avatar는 배경색만으로 자기 경계를 만든다.
 * 이니셜 Avatar의 면은 `background.grey`이고, forced-colors에서 저자 색은 시스템 색으로 갈아끼워져 페이지 배경(`Canvas`)과 같아질 수 있다.
 * 그러면 원이 사라져 이니셜이 허공에 뜬 글자가 된다 — `.ui-button--variant-contained`와 같은 결함이라 같은 전략(테두리)으로 고친다.
 * 포커스는 보지 않는다.
 * Avatar는 상호작용이 없어 `:focus-visible` 규칙 자체가 없다.
 */
describe('Avatar', () => {
  ...
  it('포커스·상호작용 규칙을 만들지 않는다', () => {
    // 정적 visual이다. 상호작용 표시가 생기면 소비자가 누를 수 있다고 오해한다.
...
/**
 * Badge는 면 색만으로 자기를 식별시킨다.
 * forced-colors에서 면과 링이 모두 `Canvas`로 평탄화될 수 있고, 그러면 dot은 통째로 사라지고 count는 숫자만 허공에 남는다.
 * 그 모드에서만 `CanvasText` 테두리를 켠다 — Avatar·contained 버튼과 같은 전략이다.
 * 포커스는 보지 않는다.
 * Badge는 상호작용이 없어 `:focus-visible` 규칙 자체가 없다.
 */
describe('Badge', () => {
  ...
  /**
   * overlay가 앵커의 클릭·탭을 삼키면 안 된다.
   * 이 컴포넌트에서 가장 조용하게 깨지는 자리라 선언 자체를 못박는다 — jsdom은 specificity를 계산하지 않아 런타임 확인이 믿을 수 없다.
   */
  ...
  /**
   * 위치는 논리 방향이어야 RTL에서 뒤집힌다.
   * `transform: translate`는 물리 좌표라 RTL에서 배지가 앵커 안쪽으로 들어간다 — 그래서 쓰지 않는다는 결정을 고정한다.
   */
...
/**
 * Chip은 선택 상태를 forced-colors에서 잃기 쉽다.
 * selected를 면·테두리 색으로만 표현하면 그 모드에서 둘 다 시스템 색으로 평탄화되어 어느 chip이 선택됐는지 보이지 않는다.
 * `SegmentControl`과 같은 전략으로 그 모드에서만 `Highlight`로 채운다.
 * passive chip에 상호작용 규칙이 새지 않는 것도 함께 못박는다 — 누를 수 없는 것이 눌릴 것처럼 보이면 안 되고, 그 경계는 셀렉터에만 있어서 리팩터링에 쉽게 무너진다.
 */
describe('Chip', () => {
  ...
  /** 상호작용 규칙은 전부 `--interactive`를 요구해야 한다 — passive에 새면 안 된다 */
  ...
  /**
   * pressed는 색이 아니라 위치다.
   * `component.pressedOffset`을 쓰는 것이 이 저장소의 정본이고 `contrast.test.ts`의 `pressed 상태 어휘`가 `color.*pressed*` 토큰 생성을 막는다.
   */
...
/**
 * List는 상호작용하지 않는다.
 * forced-colors 블록이 없어야 정상이다 — 색도, 포커스도, 상태도 없어서 그 모드에서 잃을 것이 없다.
 * 대신 이 컴포넌트가 지켜야 하는 경계를 못박는다.
 * 상호작용 규칙이 들어오면 `ListItem`이 clickable div로 흐르기 시작한 신호다.
 */
describe('List', () => {
   ...
   * `rules()`로 셀렉터를 정확히 비교하지 않는다 — sass가 비ASCII 주석 때문에 `@charset "UTF-8";`를 맨 앞에 붙이고, 그 파서는 그것을 첫 규칙의 셀렉터에 붙여 읽는다 (`.ui-list`가 `@charset "UTF-8"; .ui-list`가 된다).
   * 다른 컴포넌트 검사들은 첫 규칙을 이름으로 집지 않아서 드러나지 않았다.
   * 여기서는 규칙 블록을 직접 범위 지정한다.
   */
...
/**
 * Table은 격자로 값과 열의 관계를 보여 준다.
 * 셀 경계를 `stroke.light`로 그리는데 forced-colors에서 저자 색이 시스템 색으로 평탄화되면 격자가 사라져 어느 값이 어느 열인지 읽을 수 없다.
 * 그 모드에서만 `CanvasText`로 되살린다.
 * 정렬 방향 표시자도 함께 본다 — 테두리로 만든 삼각형이라 그 모드에서 남고, `content`로 글자를 넣지 않아 스크린리더가 `aria-sort`와 중복해서 읽지 않는다.
 */
describe('Table', () => {
  ...
  it('포커스 표시가 outline 으로 선언되어 있다', () => {
    ...
    // 정렬 button과 스크롤 영역 둘 다 포커스를 받는다.
  ...
  /**
   * 방향 표시는 `aria-sort`가 유일한 진실이어야 한다.
   * 별도 시각 플래그를 두면 접근성 상태와 시각이 어긋난 상태가 만들어진다.
   * sass가 속성 선택자의 따옴표를 지우므로(`[aria-sort=ascending]`) 따옴표를 강제하지 않는다.
   */
  ...
  /** 행 선택·hover는 선택 가능한 table의 개념이고 V1 범위가 아니다 */
  ...
  /** caption을 지우면 table의 접근 가능한 이름이 사라진다 — 시각만 숨긴다 */
  it('hiddenCaption 이 접근성 트리에서 caption 을 지우지 않는다', () => {
    ...
    for (const rule of hidden) {
      ...
      // 저장소 정본 패턴이다 (`form-control`의 hiddenLabel).
...
/**
 * SkipLink는 포커스를 받을 때만 드러나는 우회 링크다.
 * 그 모드에서 포커스 표시를 잃으면 링크가 있다는 사실 자체가 사라진다.
 * `SHEETS`의 규칙(그림자에만 기댄 포커스에는 forced-colors 대응이 있다)으로는 볼 수 없다.
 * 이 시트는 `.ui-button:focus-visible`과 같은 전략을 골랐기 때문이다 — outline과 그림자를 함께 선언해 그림자가 지워져도 outline이 남는다.
 * 그래서 전용 블록 대신 outline이 실제로 선언되어 있는지를 본다.
 */
describe('SkipLink', () => {
...
/**
 * SegmentControl은 두 상태를 forced-colors에서 잃기 쉽다.
 * - 포커스: 그림자만으로 그리면 지워진다 — SkipLink·버튼과 같은 전략으로 outline을 선언한다
 * - 선택: 배경·글자색만으로 표현하므로 시스템 색으로 평탄화되면 어느 세그먼트가 눌렸는지 보이지 않는다 — 그 모드에서만 시스템 색(`Highlight`)으로 채운다
 */
...
/**
 * Checkbox·Radio는 `appearance: none`인 native input을 토큰 색으로 칠한다.
 * forced-colors에서는 그 배경·테두리 색이 시스템 색으로 평탄화되어 checked·indeterminate가 사라진다.
 * 그 모드에서는 native 외형으로 되돌린다 — 상태를 OS가 시스템 색으로 직접 그린다.
 * `forced-color-adjust: none`으로 저자 색을 되살리지 않는다 — 사용자가 고른 고대비 팔레트를 끄는 셈이다.
 */
describe.each([
...
/**
 * Switch는 Checkbox·Radio처럼 native 외형으로 되돌리지 않는다.
 * native 체크박스에는 thumb이 없어서 "켜짐/꺼짐 위치"라는 이 컨트롤의 표현이 사라진다.
 * 대신 forced-colors에서 살아남는 수단만으로 그린다.
 * - thumb은 border로 채운다 — 그 모드에서 배경색은 Canvas로 지워지지만 테두리는 남는다
 * - 켜짐/꺼짐은 색이 아니라 thumb 위치가 1차로 말하고, 그 위에 시스템 색을 얹는다 (checked는 `Highlight`, disabled는 `GrayText`)
 * - 포커스는 outline이다
 */
describe('Switch', () => {
...
/**
 * aria-activedescendant 목록의 활성 option은 DOM 포커스가 아니라서 UA 포커스 링이 없다.
 * 배경 틴트(`field.surfaceSubtle`)는 패널과 1.1:1 안팎이라 활성 위치를 보여 주지 못하고 forced-colors에서는 사라진다.
 * 그래서 활성 option은 outline을 선언해야 한다.
 */
describe.each([
