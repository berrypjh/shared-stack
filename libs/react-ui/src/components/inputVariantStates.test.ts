/**
 * 세 입력 variant의 시각 상태 매핑 계약.
 * 목표 우선순위는 RN 리졸버(`InputBase.styles.test.ts`)와 같다 (`disabled > error > focused > 평상시`).
 * RN은 그것을 함수로 계산하지만 web은 CSS가 정한다 — 그래서 "어느 규칙이 이기나"를 선언 순서에 맡기면 파일을 재정렬하는 것만으로 조용히 바뀐다.
 * 그래서 값을 묻지 않고 매칭되는 상태 규칙이 정확히 하나인지를 본다.
 * 서로 배타적이면 결과가 특정도와 선언 순서 어느 쪽에도 좌우되지 않는다 (`test/componentStyles.ts`가 InputLabel·FormHelperText에 같은 성질을 쓴다).
 * 규칙은 CSSOM이 아니라 컴파일된 CSS 텍스트에서 읽는다 — jsdom의 CSSOM은 shorthand 안에 `var()`가 들어간 선언(`border: var(--x) solid var(--y)`)을 만나면 그 규칙을 통째로 버린다.
 * 매칭 판정만 jsdom에 맡긴다 — `element.matches()`는 `:not()`과 복합 셀렉터를 제대로 다룬다.
 */
...
/**
 * `@media` 블록과 주석을 걷어낸다.
 * 상태 우선순위는 미디어 쿼리 밖에서 정해진다.
 * 주석을 지우는 이유는 다르다 — 아래 파서가 `{` 앞을 셀렉터로 보기 때문에, 규칙 위 주석이 남으면 셀렉터에 섞여 `matches()`가 깨진다.
 */
...
/**
 * 상태 클래스를 언급하는 규칙만.
 * 평상시(base) 규칙과 `:hover`는 뺀다 — hover는 이미 `:not()`으로 배타적이고, 상태끼리의 다툼이 아니다.
 */
...
describe.each(VARIANTS)('%s variant', (_name, root, sheet) => {
  ...
  /**
   * 테두리 색의 임자는 상태 조합마다 정확히 하나여야 한다.
   * 둘 이상이 매칭되면 누가 이기는지가 선언 순서에 달리고, 그것은 계약이 아니라 우연이다.
   */
  ...
  /**
   * 포커스 halo는 비활성 입력에 남으면 안 된다.
   * `focused`와 `disabled`는 독립된 boolean이라 (`InputBase.utils.ts`의 클래스 목록) 두 클래스가 함께 붙는 상태가 실재한다.
   * halo 규칙이 `disabled`를 배제하지 않으면 비활성인데 포커스처럼 보이는 입력이 나온다.
   */
