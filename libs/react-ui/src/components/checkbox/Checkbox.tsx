 ...
 * checked·defaultChecked·disabled·required·name·value·form·form reset·FormData·Space·라벨 클릭은 전부 native가 소유한다.
 * 컴포넌트는 체크 상태를 들지 않는다.
 * - 루트는 `<label>`이라 보이는 라벨을 누르면 토글된다. `className`·`style`은 루트로, 나머지 prop과 `ref`는 input으로 간다
 * - `indeterminate`는 DOM property라 렌더마다 동기화한다. attribute로 새지 않는다
 * - FormControl에서는 `disabled`·`error`만 상속한다. `required`는 그룹의 필수와 뜻이 달라서, `size`·`color`는 대응하는 토큰이 없어서 받지 않는다
 */
export const Checkbox = ({
  ...
  // 의존성 배열이 없는 것이 의도다 — 클릭이 property를 지워도 다음 렌더에서 prop이 진실이 된다.
  useLayoutEffect(() => {
