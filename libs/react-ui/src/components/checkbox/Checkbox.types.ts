...
export type CheckboxOwnProps = {
  /**
   * 선택도 비선택도 아닌 혼합 상태.
   * HTML attribute가 없고 DOM property로만 존재한다.
...
/**
 * native `<input type="checkbox">`의 prop을 그대로 받는다.
 * `type`은 Checkbox가 소유한다.
 * 접근 가능한 이름을 판별 유니온으로 강제한다 — 보이는 라벨(`children`)이 있으면 그 글자가 이름이고, 없으면 `aria-label`·`aria-labelledby` 중 하나가 필수다.
 * 보조 타입을 이름 붙여 교차시키지 않는다 — `dts-bundle-generator`가 그 이름을 공개 선언으로 끌어올린다.
 */
