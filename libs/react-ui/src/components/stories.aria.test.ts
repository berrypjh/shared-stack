 ...
 * `aria-describedby="x"`는 같은 문서에 `id="x"`가 없으면 조용히 아무것도 하지 않는다 — 타입도 렌더도 통과하고, 스크린리더에게만 설명이 사라진다.
 * 실제로 7개 스토리가 설명 요소를 렌더하지 않은 채 이 속성만 달고 있었고, axe는 그것을 `aria-valid-attr-value` incomplete로 보고했다.
 * `storybook:a11y`가 런타임에서 같은 결함을 잡지만 정적 Storybook을 통째로 빌드해야 한다.
 * 이 검사는 소스만 읽어 몇 밀리초 안에 끝나므로 빌드 전에 먼저 걸린다.
 * 파싱이 아니라 텍스트 스캔이다.
 * 스토리는 리터럴 id를 쓰는 것이 관례라 그 관례를 검사하며, 템플릿 리터럴로 만든 id(`id={`tm-${name}-error-helper`}`)도 같은 모양으로 수집한다.
 */
...
/**
 * 검사 대상 속성.
 * `aria-*`는 값이 공백으로 나뉜 id 목록일 수 있고, `htmlFor`는 하나뿐이다 — 공백 분리는 후자에 무해하다.
 * 어디를 가리키지도 않는 `<label htmlFor>`도 같은 결함이라 함께 본다.
 */
