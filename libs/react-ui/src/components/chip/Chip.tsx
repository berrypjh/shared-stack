 ...
 * `ChipProps`가 이 타입을 참조하지 않으므로 `dts-bundle-generator`가 공개 선언으로 끌어올리지 않는다 (`TextFieldInputModeProps`가 새지 않아야 하는 것과 같은 이유).
 */
 ...
 * - interactive (`onClick` 있음): native `<button type="button">`, `selected`를 함께 주면 `aria-pressed` toggle이 된다
 * native button을 재구현하지 않는다.
 * `div role="button"`은 Enter/Space 활성화, `disabled`의 클릭·키보드·탭 순서 차단, 폼 밖 동작을 전부 손으로 다시 만들어야 하고, 그중 하나라도 빠지면 조용히 접근성이 깨진다.
 * 브라우저가 이미 옳게 하는 일이다.
 * `selected`를 주지 않으면 `aria-pressed`를 달지 않는다 — 없는 토글 시맨틱을 지어내면 스크린리더가 "누름 안 됨"을 읽어 상태가 있다고 오해하게 만든다.
 * `leading` 슬롯은 `aria-hidden`이다.
 * interactive 모드에서 접근 가능한 이름은 내용에서 계산되므로, 글리프 문자("◆")가 트리에 남으면 이름이 "◆ 필터"로 오염된다.
 * 라벨이 이름이다.
...
export const Chip = (props: ChipProps) => {
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      // toggle일 때만 단다. `selected`가 undefined면 단순 action chip이다.
