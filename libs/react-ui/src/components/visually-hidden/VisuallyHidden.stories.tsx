 ...
 * 그래서 story는 "보이는 것"이 아니라 읽히는 이름을 보여준다.
 * 아래 예시들은 화면에서 아무것도 달라 보이지 않지만, 접근성 패널(또는 스크린리더)에서 컨트롤의 이름이 생긴다.
 * 포커스로 드러나는 story는 없다 — 그 동작은 `SkipLink`의 책임이다.
 * 포커스 가능한 자식을 감싸는 예시도 두지 않는다 — 키보드로는 갈 수 있는데 끝까지 보이지 않는 상태가 되기 때문이다.
 */
const meta = {
 ...
 * 아이콘은 `aria-hidden`으로 접근성 트리에서 빼고, 이름은 숨은 글자가 만든다.
 * `aria-label`과 달리 실제 텍스트 노드라서 브라우저 번역·페이지 내 검색에 걸린다.
 * 라이브러리의 `IconButton`에는 이 패턴이 필요 없다 — 그쪽은 타입이 `aria-label` 또는 `aria-labelledby`를 강제해서 이름 없는 상태를 아예 만들 수 없다.
 * VisuallyHidden이 필요한 곳은 그런 강제가 없는 호스트 요소(`<button>`·`<th>` 등)다.
 */
 ...
 * 아이콘 열은 보는 사람에게 설명이 필요 없지만, 헤더 셀이 비어 있으면 스크린리더가 그 열을 이름 없이 읽는다.
 */
