...
describe('<VisuallyHidden />', () => {
  ...
  describe('root', () => {
    /**
     * `<span>`이다 — 구문 내용(phrasing content)이라 버튼·라벨 안에 들어가 접근 가능한 이름에 기여할 수 있다.
     * 블록 요소면 그 자리에 넣을 수 없다.
     */
    it('span 으로 렌더링한다', () => {
   ...
   * 그래서 접근성 트리에서 지우는 수단을 자동으로 붙이지 않는다.
   * 붙이는 순간 컴포넌트가 아무 일도 하지 않는 것과 같아진다 — 보이지도 않고 읽히지도 않으면 그냥 지운 것이다.
   */
  describe('접근성 계약', () => {
     ...
     * `aria-label` 대신 이것을 쓰는 이유는 실제 텍스트 노드라서 번역·글자 선택·검색에 걸리고, 브라우저 번역기가 `aria-label`보다 안정적으로 다루기 때문이다.
     */
     ...
     * 이름 계산은 텍스트 노드를 그대로 이어 붙인다 — JSX 줄바꿈은 공백을 만들지 않으므로 기대값도 붙여 적는다.
     * 숨은 글자에 구분 기호를 직접 넣는 이유다.
     */
  ...
  describe('스타일 계약', () => {
    /** jsdom이 색 키워드를 `rgb()`로 정규화하므로 기대값도 그 형태로 적는다 */
   ...
   * jsdom의 CSSOM은 `clip`과 논리 속성을 신뢰할 수 있게 다루지 못해서 계산값 대신 텍스트를 본다 (`skip-link`·`switch`·`divider`와 같은 이유).
   */
  describe('visually-hidden.scss', () => {
    ...
    /**
     * 접근성 트리에서 지우는 두 가지를 쓰지 않는다.
     * `table.scss`의 hiddenCaption 검사와 같은 계약이다.
     */
    ...
    /**
     * 포커스로 드러나는 동작은 SkipLink의 책임이다.
     * 여기에 `:focus-within` 드러내기를 넣으면 두 컴포넌트가 같은 일을 하게 되고, 무엇보다 VisuallyHidden 안의 포커스 가능한 자식은 "포커스는 가는데 보이지 않는" 위험한 상태가 된다.
     * 그 용도는 SkipLink로 보낸다.
     */
