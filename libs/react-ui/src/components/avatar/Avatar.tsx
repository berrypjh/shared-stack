...
/**
 * 정적 identity visual.
 * 이미지가 있으면 `<img>`로, 없거나 실패하면 fallback으로 그린다.
 * 접근성의 핵심은 같은 정보를 두 번 읽히지 않는 것이다.
 * - 이미지와 fallback은 동시에 렌더되지 않는다 — 둘 중 하나만 DOM에 있으므로 구조적으로 중복이 불가능하다 (`aria-hidden`으로 가리는 것보다 확실하다)
 * - `alt`를 준 fallback은 루트가 `role="img"` + `aria-label`로 이름을 갖고 시각 글자는 `aria-hidden`으로 감춘다 — 이니셜("길동")이 이름("홍길동")과 함께 읽히면 같은 사람을 두 번 말하게 된다
 * 이름을 지어내지 않는다 — `alt`가 없으면 이미지는 장식이 되고 fallback 글자가 그대로 읽힌다.
 * 상호작용이 없어 `tabIndex`·`role="button"`·hover/pressed/focus 상태를 만들지 않는다.
 * `displayName`도 두지 않는다 — 최상위 속성 할당은 번들러가 순수하다고 증명하지 못해 tree-shaking을 막는다 (`.size-limit.cjs` 머리말, Checkbox·Radio·Switch와 같은 관례).
 */
export const Avatar = ({
  src,
  alt,
  ...
  // 실패를 URL에 매어 둔다 — `src`가 바뀌면 effect 없이도 깨끗하게 다시 시도된다.
  ...
  return (
    <span
      ...
      // fallback이 이름을 대신 진다. 이미지가 없으므로 루트가 그 그래픽이다.
