...
describe('<Box />', () => {
  ...
  describe('visuals', () => {
     ...
     * 경로 검증은 타입이 한다 (`ColorToken`은 color 트리의 leaf 경로다).
     * 타입을 우회해 들어온 값에 대해 web은 이름을 평탄화해 넘기기만 하고, 브라우저는 정의되지 않은 변수를 만나 그 선언을 버린다 — 배경이 칠해지지 않을 뿐 아무것도 깨지지 않는다.
     * 같은 입력에서 RN은 던진다 (`getToken`의 엄격 실패, `ui-core/src/tokens/getToken.test.ts`).
     * 갈리는 이유는 수단이 다르기 때문이다 — RN은 렌더 시점에 토큰 트리를 실제로 조회해서 결손이 `undefined`인 채로 색 자리까지 흘러가지만, web은 조회하지 않고 이름만 넘긴다.
     * 대칭을 만들려고 web에 런타임 검증을 더하지 않는다 — 타입이 이미 막는 입력을 위해 모든 소비자 번들이 비용을 내게 된다.
     * 이 테스트는 그 결정을 고정한다.
     */
