...
/** 기본 패널 질의. 기본 시맨틱에는 role이 없어서 testid로 찾는다. */
...
describe('<Popover />', () => {
  ...
  /**
   * 트리거의 `aria-haspopup`과 패널의 `role`은 하나의 출처에서 나와야 한다.
   * 예전에는 둘이 각자 기본값(`'dialog'` / `asDialog=true`)을 가져서 라이브러리 기본값만으로도 서로 어긋날 수 있었다.
   */
  describe('시맨틱 (semantics)', () => {
    it('기본은 disclosure — 트리거에 aria-haspopup 이 없고 패널에 role 이 없다', () => {
      ...
      // `aria-haspopup="true"`는 ARIA에서 menu와 같은 뜻이라 generic disclosure에 쓰면 거짓말이 된다.
      // disclosure는 aria-expanded만으로 표현한다.
  ...
  /**
   * `useId`로 만든 id는 인스턴스마다 달라야 하고 리렌더를 건너 살아남아야 한다.
  ...
  describe('panel', () => {
    ...
    it('컨슈머 id 는 panelId 로 덮어써진다', () => {
      ...
      // aria-controls가 반드시 이 id를 가리켜야 해서 컴포넌트가 소유한다.
  ...
  /**
   * 포커스 수명주기.
   * 규칙은 하나다 — 닫힐 때 포커스가 패널 안에 있었다면 트리거로 되돌리고, 아니면 건드리지 않는다.
   * 이 규칙 하나가 네 경우를 모두 덮는다 — Escape(패널 안이면 복원), 트리거 클릭(이미 트리거에 있으니 그대로), 바깥 클릭(사용자가 고른 컨트롤을 빼앗지 않음), 컨트롤드 외부 종료(패널 안이면 복원).
   * 정책 표도 오버레이 스택도 필요 없다.
   */
  describe('포커스 수명주기', () => {
    ...
    it('controlled 외부 종료에서도 패널 안의 포커스를 되돌려야 한다', () => {
      // 바깥 포인터가 아니라 컨슈머가 프로그래매틱하게 닫는 경우다.
      // 포커스가 패널 안에서 파괴되므로 되돌려야 한다.
      const { setProps } = render(
        <Popover open>
          <PopoverTrigger>
            <button type="button">Open</button>
  ...
  /**
   * 여러 인스턴스.
   * 한 번의 상호작용이 무관한 팝업의 상태를 바꾸면 안 된다.
   * 전역 오버레이 스택 없이 로컬 규칙 둘로 푼다.
   * - 내 패널 안에 열린 패널이 또 있으면 안쪽에게 양보한다
   * - 포커스가 다른 패널 안에 있으면 그쪽이 주인이다
  ...
  describe('다중 / 중첩', () => {
    ...
    /**
     * 중첩에서 가장 흔한 포커스 위치는 안쪽 트리거다 — 방금 그것을 눌러 안쪽을 열었기 때문이다.
     * 그 트리거는 바깥 패널 안에 있고 안쪽 패널 밖에 있어서, 패널 포함 여부만 보면 어느 쪽도 자기 것이라고 판단하지 못한다.
     */
    ...
    /**
     * Escape의 주인은 이벤트가 어디서 났는지로 정해진다 — `document.activeElement`를 묻지 않는다.
     * 아래 둘은 user-event의 포커스 해석을 변수에서 빼고 DOM 이벤트를 직접 쏴서 그 메커니즘 자체를 고정한다.
     * 예전 구현은 포커스를 추론했고, 그 추론이 jsdom과 실제 브라우저에서 갈려 중첩에서 아무것도 닫히지 않았다.
     */
    ...
    it('팝업 안에서 난 Escape 는 document 까지 올라가지 않는다', () => {
      ...
      // 막지 않으면 무관한 형제 팝업까지 같은 Escape로 닫힌다.
