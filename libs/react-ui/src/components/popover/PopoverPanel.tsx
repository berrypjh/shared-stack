 ...
 * 여기까지 온 Escape는 어떤 팝업 안에서도 일어나지 않은 것(문서 어딘가)이라, 그때의 규칙은 "중첩이면 안쪽부터"뿐이다.
 */
...
export const PopoverPanel = ({ children, asDialog, className, ...rest }: PopoverPanelProps) => {
  ...
  /**
   * 바깥 클릭과 Escape로 닫기.
   * Escape의 주인을 고르는 방식이 핵심이다.
   * 예전에는 `document.activeElement`와 `aria-controls`로 "이 포커스가 누구 것인지"를 추론했는데, 그 추론이 jsdom과 실제 브라우저에서 갈렸다 — 중첩에서 안쪽 트리거에 포커스가 있을 때 브라우저에서는 아무도 자기 것이라고 하지 않아 Escape가 통째로 먹혔다.
   * 그래서 추론을 버리고 DOM 버블링에 맡긴다.
   * 리스너를 내 패널과 내 트리거에 직접 달면, 키 이벤트가 내 팝업 안에서 났을 때만 불린다.
   * 중첩이면 가장 안쪽 것이 먼저 불리고 `stopPropagation()`으로 바깥을 막는다 — 순서가 DOM 명세로 보장되므로 환경을 타지 않는다.
   * 어느 팝업 안에서도 나지 않은 Escape(문서 어딘가에 포커스)만 document까지 올라온다.
   * 그때는 열린 팝업이 함께 닫히는 light-dismiss이고, 중첩은 안쪽부터다.
   */
  useEffect(() => {
   ...
   * 그래서 열려 있는 동안 `focusin`으로 "안에 있었는지"를 계속 따라간다.
   * 포커스가 밖으로 나가면 새 대상에서 focusin이 떠서 false가 되고, 요소가 제거돼 포커스가 파괴된 경우엔 focusin이 뜨지 않아 true로 남는다.
   * 되돌릴 값이 필요한 쪽만 정확히 true다.
   */
  useEffect(() => {
    if (!open) return undefined;
    ...
    return () => {
      ...
      // cleanup 시점의 현재 트리거를 읽어야 한다 — effect 시작 때 잡아두면 그 사이 트리거가 다시 마운트된 경우 분리된 옛 노드를 붙잡는다.
      // 그래서 exhaustive-deps의 ref 경고는 여기서 의도적으로 무시하고, 대신 isConnected로 직접 확인한다.
